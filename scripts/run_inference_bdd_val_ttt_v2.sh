...
SAM2_CFG="${SAM2_CFG:-sam2/configs/sam2.1/sam2_ttt_inference_l_restricted_v2.yaml}"
SAM2_CHECKPOINT="${SAM2_CHECKPOINT:-/root/autodl-tmp/ttt_rr/logs/sam_ttt_bdd_restricted_v2_run1/checkpoints/checkpoint.pt}"
OUTPUT_MASK_DIR="${OUTPUT_MASK_DIR:-/root/autodl-tmp/output_bdd_semi_val/bdd_restricted_ttt_v2}"
LOG_FILE="${OUTPUT_MASK_DIR}/inference.log"
RUN_EVAL_AFTER_INFERENCE="${RUN_EVAL_AFTER_INFERENCE:-1}"

mkdir -p "$OUTPUT_MASK_DIR"

echo "Running BDD val inference with improved TTT enabled..."
